...
SUPPORTED_STORYBOARD_PROVIDERS = frozenset({"alibaba", "openai"})
SYSTEM_PROMPT = (
...
def resolved_storyboard_model(config: AppConfig) -> str:
    provider = config.talemotion_storyboard_provider.strip().lower()
    model = (config.talemotion_storyboard_model or "").strip()
    if provider not in SUPPORTED_STORYBOARD_PROVIDERS:
        raise ValueError("Storyboard provider must be alibaba or openai.")
    if not model:
        raise ValueError("Storyboard model must not be empty.")
...
def _configured_model(config: AppConfig) -> Model:
    identifier = resolved_storyboard_model(config)
    provider_name = config.talemotion_storyboard_provider.strip().lower()
...
class PydanticAIStoryboardGenerator:
    ...
    def generate(
        ...
        *,
        ...
        if missing and self.model is None:
            raise SceneMediaError(
                "missing_configuration",
        ...
        try:
            agent = self.agent_factory(
                self.model or _configured_model(self.config),
...
def create_storyboard_generator(config: AppConfig) -> StoryboardGenerator:
    return PydanticAIStoryboardGenerator(config)
